...
{
    /**
     * The main method interacts with player by welcoming him/her to the BlackJack game 
     * simulator, prompts for user input of name and initial bankroll amount, updates the 
     * player's bankroll after each game by using the results returned by the method 
    ...
    public static void main( String[] args )
    {
        ...
        while( bank > 0 && playerContinue )
        {
            ...
            outcome = playBJ( name, betAmount ); 

            // for each hand j generated due to card pair splitting
            for( int j = 0; j < outcome.size(); j++) 
            {   // switch case statment for each outcome possibility
                switch( outcome.get(j).get(0).intValue() ) 
                {  // Natural Black Jack on players initial two cards
                    case 0:
                    {
                        System.out.println(" ");
                    ...
                    // Player Wins
                    case 2:
                    {   // pays +1.0x player earns orig bet amount
                        System.out.println(" ");
                        System.out.print( name +  "_" + j + " Won!");
                        updateAmount = betAmount; 
                        /* Accounts for insurance against Dealer BlackJack when a Dealer 
                         * initially has a face-up Ace. If Dealer has face up Ace, if player
                         * selected insurance of half the original bet amount, but dealer 
                         * did not get BlackJack, and player won, then player loses insurance
                         * amount, and wins the original bet amount. */
                         
                         // If had insurance on A, and player wins
                        if( outcome.get(j).get(1).intValue() > 0 ) 
                        {  // pays +0.5x player earns half original bet amount
                    ...
                    case 3:
                    {
                        ...
                        /* Accounts for insurance against Dealer BlackJack when a Dealer 
                         * initially has a face-up Ace. If Dealer has face up Ace, if 
                         * player selected insurance of half the original bet amount, 
                         * and dealer did get BlackJack, then player loses original bet
                         ...
                         * the dealer still won, then player loses insurance amount, and 
                         * the original bet amount. */
                         
                        //Dealer does not have BJ, player still put insurance
                        else if( outcome.get(j).get(1).intValue() > 0 && outcome.get(j).get(2).intValue() == 0 )
                        {   // pays -1.5x (player loses money)
                            updateAmount = -1.5*betAmount; 
                        }
                        /* Dealer does not have initial face up Ace, and there is no 
     ...
     */
    static ArrayList< ArrayList< Double > > playBJ( String nm, double btAmnt )
    {   // Initialize Variables     
        ...
        numAceP = checkAce( player.get(0), aceP );     
        ...
        if( dealer.getCard(0).substring(0,1).equals( "A" ) )
        {
            ...
            Scanner inputInsur = new Scanner(System.in);
            System.out.println(" ");
            System.out.print("\n Enter (1) if want insurance, or (0) if not: $");
            decisionInsur = inputInsur.nextInt();
        ...
        {
            // Prompt if split
            Scanner inputSplit = new Scanner(System.in);
            System.out.println(" ");
            System.out.println(" Enter (1) to split, and (0) otherwise: ");
            decisionSplit = inputSplit.nextInt();
        ...
        for( int i = 0; i < player.size(); i++ ) 
        {   // while hand value is less than 21, prompt player "hit" or "stay"
            while( playerHandValue.get(i) < 21 ) 
            {   // Prompt player "hit" or "stay"
                ...
                numAceP = checkAce( player.get(i), aceP ); 

                // Returns number cards in hand for player hand(i)
                numCardsInHand = player.get(i).numCards();

                /* Split hands again if same card as original (10, J, Q, K are not 
                 * counted the same) Checks to see if most recent card is the same 
                 * as the original split card */
                if( player.get(i).getCard( 0 ).substring(0,1).equals(  player.get(i).getCard( numCardsInHand-1 ).substring(0,1)  ) ) 
                {
        ...
        /* Stop game, return playerResults if all player hands busted by getting frequency
         * of busts in all player's hands results. */
        ...
        else
        {
            ...
            for( int i = 0; i <  player.size(); i++ ) 
            {   // if dealer hand value greater than player hand value
     ...
     * @param facedown  a boolean value specifying whether the second card is face down
     * @return          void, but displays card representation to java console
     */
    static void displayHand( ArrayList<String> crds, boolean facedown )
    {   // Initialize variables for displaying wall dividers, roof and floor for card
        String wallDisp = "";
        String roofFloor = "";
        // wall
        if( facedown == true )
    ...
    /**
     * This method splits a pair of identical cards, and imports them into separate hands.
     * @author Cindi
     * @version 1.0
    ...
    static int hitStay()
    {
        ...
        Scanner inputHitStay = new Scanner(System.in);
        System.out.println(" ");
    ...
    /**
     * This method checks the player's hand to see if there are any Aces, and assigns 
     * user input values.
     * @author Cindi
     * @version 1.0
     * @param plyr    a player's Hand object
     * @param a       an empty ArrayList for saving the values assigned to "A" 
     *                (either 1 or 11)
     * @return        the number of "A" in hand
     */
    static int checkAce( Hand plyr, ArrayList<Integer> a )
    {
    ...
    /**
     * This method checks to see if there are Aces in the dealer's hand, and automatically
     * assigns values to the Aces based on what is best for the dealer's interests.
     * @author Cindi
     * @version 1.0
     * @param plyr    a dealer's Hand object
     * @param a       an empty ArrayList that will contain the values of the "A's"
     */
    static int checkAceDealer( Hand plyr, ArrayList<Integer> a )
    {   // Initialize variables
        ...
        for( int i = 0; i < plyr.numCards(); i++ ) 
        {   // if A located at i
            if( plyr.getCard(i).substring(0,1).equals( "A" ) )  
            {   //index of A in dealer hand 
                acePos.add( i ); 
            }
        ...
        {   // total value of cards in hand excluding Ace, Ace is initialized to 0
            ...
            {   // Sum of Ace possibilities
                /*
                 * numA = 2
                 * 11, 11 = (numA-0)*11  +  (numA - (numA-0))*1 = sum_A_possibilities.get(i)
                 * 11, 1  = (numA-1)*11  +  (numA - (numA-1))*1 = sum_A_possibilities.get(i)
                 * 1,  1  = (numA-2)*11  +  (numA - (numA-2))*1 = sum_A_possibilities.get(i)
                 *
                 * numA = 3
                 * 11, 11, 11 = (numA-0)*11  +  (numA - (numA-0))*1 = sum_A_possibilities.get(i)
                 * 11, 11, 1  = (numA-1)*11  +  (numA - (numA-1))*1 = sum_A_possibilities.get(i)
                 * 11, 1, 1   = (numA-2)*11  +  (numA - (numA-2))*1 = sum_A_possibilities.get(i)
                 * 1,  1, 1   = (numA-3)*11  +  (numA - (numA-3))*1 = sum_A_possibilities.get(i)
                 */
            ...
            for( int i = 0; i < sum_A_possibilities.size(); i++ ) 
            {   // Sum of Hand including Ace
            ...
            {   // (21-pts) must be pos. or equal to 0, so pnts are less than or equal 21
                if( difference.get(i) >= 0 ) 
                {   // saves the possitive or 0 differences into ArrayList
